...
document.getElementById("roundView").style.backgroundColor = "#a3f5de";
document.getElementById("firstButton").style.background = "linear-gradient(#c9f3d4, #4bd871)";
...
function mouseDown() {
    if (playing) {
        document.getElementById("firstButton").style.background = "linear-gradient(#d84b4b, #f3c9c9)";
    } else {
        document.getElementById("firstButton").style.background = "linear-gradient(#4bd871, #c9f3d4)";
    }
}
...
function startButton() {
    if (!playing) {
        ...
        document.getElementById("firstButton").style.background = "linear-gradient(#f3c9c9, #d84b4b)";
        initSort();
    } else {
        playing = false;
...
function lineFunction(pommel, radCord, color, radius) {
    if (sortView == "roundView") {
        ...
        if (pommel) {
            ...
            ctx.beginPath();
            ctx.moveTo(Math.cos(radCord*(Math.PI*2/array.length))*(radius+array[radCord])+canvasSize/2, 
			Math.sin(radCord*(Math.PI*2/array.length))*(radius+array[radCord])+canvasSize/2);
            ctx.lineTo(Math.cos(radCord*(Math.PI*2/array.length))*(radius+array[radCord]+2*Math.PI*radius/array.length)+canvasSize/2, 
			Math.sin(radCord*(Math.PI*2/array.length))*(radius+array[radCord]+2*Math.PI*radius/array.length)+canvasSize/2);
            ctx.stroke();
        } else {
            ctx.beginPath();
            ctx.moveTo(Math.cos(radCord*(Math.PI*2/array.length))*radius+canvasSize/2, 
			Math.sin(radCord*(Math.PI*2/array.length))*radius+canvasSize/2);
            ctx.lineTo(Math.cos(radCord*(Math.PI*2/array.length))*(radius+array[radCord])+canvasSize/2, 
			Math.sin(radCord*(Math.PI*2/array.length))*(radius+array[radCord])+canvasSize/2);
            ctx.stroke();
     }
    }
...
function changeSortView(view) {
    ...
    if (sortView == "classicView") {
        document.getElementById("classicView").style.backgroundColor = "#a3f5de";
    }
    if (sortView == "gradientView") {
        document.getElementById("gradientView").style.backgroundColor = "#a3f5de";
    }
    resetImage();
}
